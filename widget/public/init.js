function init (apiKey) {
  async function handleLoadWidget () {
    ...
    const WIDGET_URL = `https://thaislsilveira-feedbacker-dashboard.netlify.app/?api_key=${apiKey}&page=${page}&fingerprint=${fingerprint.visitorId}`
    const config = { method: 'HEAD' }
    const res = await fetch(`https://backend-treinamento-vue3-green.vercel.app/apikey/exists?apikey=${apiKey}`, config)
